...
Нейрон — self-hosted агент на базе OpenClaw. Работает на Hetzner VPS, общается через Telegram, использует Claude Sonnet 4.6 как мозг, и подключён к Mac через Tailscale VPN.
...
│         ┌───────────────┼───────────────┐                  │
│         ▼               ▼               ▼                  │
│   🧠 Claude       📰 Новостной    🗄️ Память                │
│   Sonnet 4.6        агрегатор       (bank + LanceDB)         │
│   (Anthropic)     (cron + Brave)  (OpenAI embeddings)      │
│                         │                                   │
│              ┌──────────┼──────────┐                       │
│              ▼          ▼          ▼                       │
│         📊 Dual     🎲 Multi-    🎤 Groq                   │
