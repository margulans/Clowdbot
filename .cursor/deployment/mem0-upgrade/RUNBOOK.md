...
## Шаг 3: Развёртывание Qdrant (OSS-режим Mem0 через плагин)
...
## Шаг 6: Hardening hotfix для mem0ai (обязательно)

> Зачем: после обновления `@mem0/openclaw-mem0` патч в `node_modules` слетает.  
> Этот шаг делает фикс воспроизводимым и проверяемым.

```bash
cd ~/Clowdbot
bash .cursor/deployment/mem0-upgrade/hotfixes/apply-mem0-embedder-baseurl-fix.sh

# Проверка должна завершиться строкой:
# OK: mem0ai hotfix applied and verified
```

---

## Шаг 7: Конфигурация OpenClaw (через бота в Telegram)
...
## Шаг 9: Критерии приёмки (production-ready)

Чеклист считается пройденным, только если выполнены все пункты:

1. `check-mem0.sh` стабильно возвращает `OK`
2. В логах gateway нет `openclaw-mem0: capture failed` после перезапуска
3. В логах Qdrant нет `POST /collections/openclaw-mem0/points/search ... 400` из runtime-трафика
4. `memory_search` и `autoRecall` работают в новой сессии (`/new`)
5. Soak-наблюдение 72 часа:
   - `capture_failed_count = 0`
   - `qdrant_400_count = 0` (исключая ручные тесты с curl)

---

## Шаг 10: Долгосрочная фиксация (upstream)

Hotfix-скрипт из Шага 6 — это страховка на проде.  
Постоянное решение должно быть в источнике:

1. Создать upstream PR в `mem0ai`:
   - `OpenAIEmbedder` учитывает `config.baseURL`
   - `ConfigManager.mergeConfig` пробрасывает `embedder.config.baseURL`
2. После релиза upstream:
   - обновить `@mem0/openclaw-mem0`
   - убедиться, что hotfix-скрипт сообщает `ALREADY_PATCHED`/не меняет файл
3. Только после подтверждения можно убрать локальный hotfix из runbook
...
7. Сохрани файл
8. Примени hotfix: bash ~/Clowdbot/.cursor/deployment/mem0-upgrade/hotfixes/apply-mem0-embedder-baseurl-fix.sh
9. Перезапусти: systemctl --user restart openclaw-gateway
10. Через 30 сек проверь: openclaw plugins list и openclaw mem0 stats

**Ожидаемый результат:** openclaw-mem0 в списке плагинов, hotfix применён и проверен, команда openclaw mem0 stats работает
```
...
### ТЗ #3 — topK тюнинг в openclaw.json

> ⚠️ **Внимание:** поля `mmr` и `temporalDecay` плагином `@mem0/openclaw-mem0` не поддерживаются —
> добавление вызывает `must NOT have additional properties` и падение gateway.
> Применять только когда плагин официально задокументирует эти поля.

```markdown
## ТЗ для @neironassistant_bot

**Задача:** Увеличить topK для recall в конфигурации плагина openclaw-mem0
**Тип:** openclaw.json хотфикс

**Шаги:**

1. Открой ~/.openclaw/openclaw.json
2. Найди секцию plugins → entries → openclaw-mem0 → config
3. Проверь что topK = 8 (уже применено)
4. Если topK < 8 — измени на 8
5. Сохрани файл
6. Перезапусти: systemctl --user restart openclaw-gateway
7. Через 30 сек проверь: openclaw mem0 stats

**Ожидаемый результат:** openclaw mem0 stats работает, в логах нет ошибок plugin
```
