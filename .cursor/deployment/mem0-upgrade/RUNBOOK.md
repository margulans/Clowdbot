...
### ТЗ #3 — MMR re-ranking + topK тюнинг в openclaw.json

```markdown
## ТЗ для @neironassistant_bot

**Задача:** Улучшить качество recall — добавить MMR и увеличить topK в конфигурации плагина openclaw-mem0
**Тип:** openclaw.json хотфикс

**Контекст:**
Сейчас topK=5 и поиск возвращает похожие факты (избыточность).
MMR (Maximal Marginal Relevance) делает выборку более разнообразной.
...
3. Измени topK: 5 → 8
4. Добавь после searchThreshold (0.35) новое поле:
   "mmr": { "enabled": true, "lambda": 0.6 }
   (lambda 0.6 = баланс relevance vs diversity; 1.0 = только relevance)
5. Добавь после "mmr" поле:
   "temporalDecay": { "enabled": true, "halfLifeDays": 45 }
   (факты старше 45 дней получают сниженный score, но не удаляются)
6. Сохрани файл
7. Перезапусти: systemctl --user restart openclaw-gateway
8. Через 30 сек проверь: openclaw mem0 stats

**Итоговый блок config должен выглядеть:**
{
"mode": "open-source",
"userId": "margulan",
"autoRecall": true,
